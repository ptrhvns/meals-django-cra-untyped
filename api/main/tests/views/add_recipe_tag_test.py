...
def test_http_method_names():
    assert dvh.has_http_method_names(views.add_recipe_tag, ["options", "post"])


def test_permission_classes():
    permission_classes = [permissions.IsAuthenticated]
    assert dvh.has_permission_classes(views.add_recipe_tag, permission_classes)


def test_recipe_not_found(api_rf, mocker):
    path = urls.reverse("add_recipe_tag", kwargs={"recipe_id": 1})
    ...
    response = views.add_recipe_tag(request, 1)
...
def test_invalid_request_data(api_rf, mocker):
    ...
    path = urls.reverse("add_recipe_tag", kwargs={"recipe_id": recipe.id})
    request = api_rf.post(path, {})
    test.force_authenticate(request, user=user)
    request.user = user
    goo4 = mocker.patch("main.views.shortcuts.get_object_or_404", autospec=True)
    goo4.return_value = recipe
    response = views.add_recipe_tag(request, recipe.id)
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
    assert len(response.data["errors"]) > 0
    assert len(response.data["message"]) > 0


def test_adding_recipe_tag_successfully(api_rf, mocker):
    user = factories.UserFactory.build()
    recipe = factories.RecipeFactory.build(id=1, user=user)
    data = {"name": "TestTag"}
    ...
    goo4.return_value = recipe
    arts = mocker.patch(
        "main.views.serializers.AddRecipeTagSerializer", autospec=True
    ).return_value
