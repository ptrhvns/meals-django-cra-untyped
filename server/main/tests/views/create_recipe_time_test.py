...
def test_http_method_names():
    assert dvh.has_http_method_names(views.create_recipe_time, ["options", "post"])


def test_permission_classes():
    permission_classes = [permissions.IsAuthenticated]
    assert dvh.has_permission_classes(views.create_recipe_time, permission_classes)


def test_recipe_not_found(api_rf, mocker):
    path = urls.reverse("create_recipe_time", kwargs={"recipe_id": 1})
